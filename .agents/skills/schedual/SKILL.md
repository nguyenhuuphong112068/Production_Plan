...
1. **Vệ sinh (Clearning)** - `#e4e405e2` (Vàng tươi): Cảnh báo vi phạm thời gian vệ sinh giữa các lô.
2. **Vi phạm thời gian biệt trữ** - `#bda124ff` (Cam/Nâu): Thời gian chờ giữa 2 công đoạn (ví dụ chờ sấy, chờ kiểm nghiệm) vượt mức cho phép.
3. **Hạn cần hàng (Expected Date)** - `#e54a4aff` (Đỏ nhạt): Lịch dự kiến kết thúc trễ hơn hạn giao hàng hoặc ngày xuất xưởng KCS.
4. **Liên kết công đoạn trước sau (Predecessor / Successor)** - `#4d4b4bff` (Xám đen): Lô công đoạn sau bắt đầu trước khi lô công đoạn trước kết thúc (lỗi gối đầu).
5. **Nguyên liệu / Bao bì (Critical Checks)** - `#920000ff` (Đỏ sẫm): Vi phạm nghiêm trọng nhất (chưa đủ nguyên liệu, hết hạn nguyên liệu chính, chưa có bao bì, v.v).
