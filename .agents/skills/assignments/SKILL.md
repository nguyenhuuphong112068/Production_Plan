...
- 💡 **Nghi vấn chưa kết luận:** cả hai lượt 10:59:26 và 11:10:02 đều chỉ chết đúng **một** endpoint và luôn là cái thứ 3 trong hàng (`overtime` dept 15), trong khi hạn ngạch nội bộ còn trống và chỉ gửi 6 request. Nghi eO2 giới hạn **số kết nối đồng thời** chứ không chỉ giới hạn theo cửa sổ thời gian → thử `SHIFT_API_MAX_CONCURRENCY=2`. Chưa kiểm chứng được vì eO2 đang trong trạng thái chặn.
