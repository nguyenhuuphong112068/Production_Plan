                ...
                <tbody>
                    @foreach ($datas as $data)
                        ...
                        <tr>
                            ...
                            <td>
                                <div> {{ $data->name }} - {{ $data->actual_batch ?? $data->batch }} -
                                    {{ $data->market }} </div>
                            ...
                            <td class = "text-left">
                                ...
                                @if (!$data->end)
                                    <div class ="text-black font-weight-bold">
                                        {{ '(1): ' . \Carbon\Carbon::parse($data->expected_date)->format('d/m/Y') }}
                                    </div>
                                @elseif (
                                    \Carbon\Carbon::parse($data->expected_date)->toDateString() <
                                        \Carbon\Carbon::parse($data->end)->addDays(5)->toDateString())
                                ...
                                @if (
                                    ...
                                    <div class = "text-center">
                                        <button class = "btn btn-success btn-accept mt-1" data-id={{ $data->id }}
                                            data-new_expected_date={{ \Carbon\Carbon::parse($data->end)->addDays(5)->format('Y-m-d') }}>
                                            <div> Cập Nhật Ngày Dự Kiên KCS </div>
                                            <div>
                                                {{ '(' . \Carbon\Carbon::parse($data->end)->addDays(5)->format('d/m/Y') . ')' }}
                                            </div>
                                        </button>
                                    </div>
                                    <div> {{ 'Updated_by: ' . $data->order_by }} </div>
                                    <div>{{ 'Updated_date: ' }}
                                        {{ $data->order_date ? \Carbon\Carbon::parse($data->pro_feedback_date)->format('d/m/Y') : '' }}
                                    </div>
                                @endif
                            ...
                            <td class="text-left">
                                ...
                                @if ($data->end)
                                    <div
                                        class = "{{ \Carbon\Carbon::parse($data->expected_date)->toDateString() < \Carbon\Carbon::parse($data->end)->addDays(5)->toDateString() ? 'text-red' : 'text-green' }}">
                                        <b>{{ '(1): ' . \Carbon\Carbon::parse($data->end)->format('d/m/Y H:n') }} </b>
                                    </div>
                                @else
...
@endif
                                ...
                                <div>{{ 'Updated_date: ' }}
                                    {{ $data->pro_feedback_date ? \Carbon\Carbon::parse($data->pro_feedback_date)->format('d/m/Y') : '' }}
                                </div>
                            </td>
                            ...
                            <td class="text-left">
                                ...
                                <div class="input-group mx-4">
                                    ...
                                    <input class="form-check-input step-checkbox" type="checkbox"
                                        name ="has_punch_die_mold" data-id="{{ $data->id }}"
                                        id="{{ $data->id }}"
                                ...
                                <div>
                                    @if (
                                        ($department == 'EN' && $plan_feedback && \Carbon\Carbon::parse($send_date)->addDays(18)->gt(now())) ||
                                            ($department == 'EN' && $plan_feedback_leader))
                                        <textarea class="updateInput text-left" name="en_feedback" data-id="{{ $data->id }}"
                                            placeholder="EN Phản Hồi Tại Đây">
@if (!empty($data->en_feedback))
{{ $data->en_feedback }}
@endif
</textarea>
                                    @else
                                ...
                                <div>{{ 'Updated_date: ' }}
                                    {{ $data->en_feedback_date ? \Carbon\Carbon::parse($data->en_feedback_date)->format('d/m/Y') : '' }}
                                </div>
                            </td>
                            ...
                            <td class="text-left">
                                ...
                                @if (!$data->actual_CoA_date && $department == 'QC' && $plan_feedback)
                                    <input type= "date" class="updateInput" name="actual_CoA_date"
                                        value="{{ $data->actual_CoA_date }}" data-id={{ $data->id }}></input>
                                @else
                                    <b class="{{ $data->actual_CoA_date ? 'text-green' : '' }}">
                                        {{ $data->actual_CoA_date ? \Carbon\Carbon::parse($data->actual_CoA_date)->format('d/m/Y') : 'Chưa có ngày ra phiếu' }}
                                    </b>
                                @endif
                                <br>
                                <b> {{ '(2):' }}</b>
                                <div>
                                    @if ($department == 'QC' && $plan_feedback)
                                        <textarea class="updateInput text-left" name="qc_feedback" data-id="{{ $data->id }}"
                                            placeholder="QC Phản Hồi Tại Đây">
@if (!empty($data->qc_feedback))
{{ $data->qc_feedback }}
@endif
</textarea>
                                    @else
                                ...
                                <div>{{ 'Updated_date: ' }}
                                    {{ $data->qc_feedback_date ? \Carbon\Carbon::parse($data->qc_feedback_date)->format('d/m/Y') : '' }}
                                </div>
                ...
                <thead style = "position: sticky; top: 60px; background-color: white; z-index: 1020">
                    <tr>
                        ...
                        <th>
                            <div>{{ 'PHÂN XƯỞNG' }}</div>
                            <div>{{ '(1) Dự Kiến KT ĐG' }} <span class ="text-red">
                                    {{ "(chưa có:  $end_packaging lô)" }} </span> </div>
                            <div>{{ '(2) Ngày ra Hồ sơ PX' }} <span class ="text-red">
                                    {{ "(chưa có:  $actual_export_record_Count lô)" }} </span></div>
                            <div>{{ '(3) Phản Hồi' }} </div>
                        ...
                        <th>
                            ...
                            <div>{{ '(1) Tình hình CC - KM' }} <span class ="text-red">
                                    {{ "(chưa có:  $actual_Mod_Count lô)" }} </span> </div>
                            <div>{{ '(2) Phản hồi' }} </div>
                        ...
                        <th>
                            ...
                            <div>{{ '(1) Ngày ra phiếu TP' }} <span class ="text-red">
                                    {{ "(chưa có:  $actual_CoA_Count lô)" }} </span></div>
                            <div>{{ '(2) Phản hồi' }} </div>
                            {{-- @if ($department == 'QC')
                                <button class = "btn btn-success"
                                    data-toggle="modal"
                    ...
                    preConfirm: () => {
                        ...
                        if (!batchNo || !orderNo) {
                            Swal.showValidationMessage(
                                "Vui lòng điền đầy đủ số lô và số lệnh!");
                            return false;
                        }
                        ...
                        $.ajax({
                            ...
                            success: function(res) {
                                ...
                                Swal.fire({
                                    ...
                                    location
                                        .reload(); // reload ngay sau khi popup đóng
                                });
                            }
                        });
                    }
