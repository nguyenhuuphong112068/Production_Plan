<div class="table-responsive" style="max-height: 70vh; overflow-y: auto;">
    <table class="table table-bordered table-striped table-hover text-center align-middle" id="inProgressTable"
        ...
        <tbody>
            @foreach($inProgressPlans as $index => $plan)
                <tr>
                    ...
                    <td>
                        <span class="badge badge-info" style="font-size: 12px;">{{ number_format($plan->batch_qty) }}
                            {{ $plan->unit_batch_qty }}</span>
                    </td>
                    ...
                    <td>
                        @if ($plan->packaging_start && $plan->packaging_end)
                            {{ \Carbon\Carbon::parse($plan->packaging_start)->format('d/m/Y') }} -
                            {{ \Carbon\Carbon::parse($plan->packaging_end)->format('d/m/Y') }}
                        @else
                    ...
                    <td>
                        <a href="{{ route('pages.plan.production.open') }}?plan_list_id={{ \App\Models\PlanMaster::find($plan->id)->plan_list_id }}&name={{ \App\Models\PlanMaster::find($plan->id)->batch }}"
                            target="_blank" class="btn btn-sm btn-outline-primary" title="Mở kế hoạch">
