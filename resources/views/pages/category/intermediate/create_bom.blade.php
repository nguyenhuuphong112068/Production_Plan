      ...
      <div class="modal-body" style="max-height: 100%; overflow-x: auto; ">
          <div class="card-body">
            ...
            <div class="mb-3 p-3" style="background-color: #f1f3f5; border-radius: 5px; display:flex; gap:10px; align-items:center;">
                ...
                <input type="text" id="input_mms_code" class="form-control" placeholder="Nhập Mã BTP (ví dụ: IS...)" style="flex: 1;">
...
<script>
  ...
  $('#btn_fetch_mms').on('click', function() {
      ...
      if (!mmsCode) {
          alert("Vui lòng nhập mã BTP để lấy công thức!");
          return;
      }
      ...
      $.ajax({
          ...
          success: function(res) {
              ...
              if (res && res.length > 0) {
                  ...
                  alert("Không tìm thấy công thức cho mã BTP này trên MMS!");
              }
          },
          error: function(err) {
              btn.html(originalHtml).prop('disabled', false);
