                ...
                <tbody>
                    ...
                    @foreach ($datas as $data)
                        <tr data-id="{{ $data->id }}">
                            ...
                            <td> 
                                <input type="datetime-local" class="time" name="start" value = "{{\Carbon\Carbon::parse($data->start)->format('Y-m-d\TH:i') }}">
                                <input type="datetime-local" class="time" name="end" value = "{{\Carbon\Carbon::parse($data->end)->format('Y-m-d\TH:i') }}">
                            </td>
                            ...
                            <td> 
                                <input type="datetime-local" class="time" name="start_clearning"value="{{ \Carbon\Carbon::parse($data->start_clearning)->format('Y-m-d\TH:i') }}">
                                <input type="datetime-local" class="time" name="end_clearning" value = "{{\Carbon\Carbon::parse($data->end_clearning)->format('Y-m-d\TH:i') }}">
                            </td>

                            <td>
                                <input type="text" class="time" name="yields" value="{{$data->Theoretical_yields}}"  oninput="this.value = this.value.replace(',', '.').replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');">
                            </td>
...
<script>
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('.btn-finised').forEach(btn => {
        btn.addEventListener('click', function (e) {
            ...
            return
            // Gửi AJAX request lên Laravel
            fetch("{{ route('pages.Schedual.finised.store') }}", {
                method: "POST",
                headers: {
