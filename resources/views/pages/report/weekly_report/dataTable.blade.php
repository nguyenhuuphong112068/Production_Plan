...
<div class="content-wrapper">
    ...
    <div class="card">
        ...
        <div class="card-body mt-4">
            ...
            <div class ="row mx-2 mb-2">
                <form id="filterForm" method="GET" action="{{ route('pages.report.weekly_report.index') }}"
                        ...
                        @foreach ($datas as $data)
                            @if ($current_Stage !== null && $current_Stage != $data->stage_code)
                                ...
                                <tr class="stage-row-total" data-stage="{{ $current_Stage }}"
                                    ...
                                    <td colspan="6" class="text-right text-end">Tổng
                            ...
                            <tr class="stage-child stage-{{ $data->stage_code }}">
                                ...
                                {{-- shift --}}
                                <td>
                                    <input type="text" class="time" name="shift" data-id="{{ $data->id }}" value="{{ $data->shift }}" {{ $auth_update }}>
                                </td>

                                {{-- day in week --}}
                                <td>
                                    <input type="text" class="time" name="day_in_week" data-id="{{ $data->id }}" value="{{ $data->day_in_weeks }}" {{ $auth_update }}>
                                </td>
    ...
    <script>
        $(document).ready(function() {
            ...
            // Xử lý tìm kiếm (nếu bạn muốn thêm ô tìm kiếm trong tương lai)
            // Hiện tại tôi cập nhật logic toggle để khớp với Monthly Report
        });
