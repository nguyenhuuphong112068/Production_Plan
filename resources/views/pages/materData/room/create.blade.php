...
<div class="modal fade" id="createModal" tabindex="-1" role="dialog" aria-labelledby="ModalLabel" aria-hidden="true">
    <div class="modal-dialog" role="document">
        ...
        <form action="{{ route('pages.materData.room.store') }}" method="POST">
            @csrf
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    @error('main_equiment_name', 'createErrors')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror

                    {{-- NAME capacity --}}
                    <div class="form-group">
                        <label for="capacity">Công Suất Máy</label>
                        <input type="text" class="form-control" name="capacity" value="{{ old('capacity') }}">
                    </div>
                    @error('capacity', 'createErrors')
                        <div class="alert alert-danger">{{ $message }}</div>
                    @enderror
