        ...
        <div class="d-flex justify-content-between align-items-center">
            ...
            <div class="d-flex align-items-center">
                ...
                <a class="btn btn-sm btn-outline-primary shadow-sm ml-2"
                    href="{{ route('pages.assignment.maintenance.weekly') }}?group_code={{ $group_code }}&reportedDate={{ $reportedDate }}"
                                                ...
                                                <td style="width: 100px">
                                                    <div class="d-flex flex-column align-items-center">
                                                        <select class="form-control form-control-sm shift-select mb-1"
                                                            {{ !$canEdit ? 'disabled' : '' }}>
                                                            <option value="1"
                                                                {{ $group_code == '11' ? 'selected' : '' }}>1
                                                            </option>
                                                            <option value="2">2</option>
                                                            <option value="3">3</option>
                                                            <option value="6">4</option>
                                                            <option value="4"
                                                                {{ $group_code == '11' ? '' : 'selected' }}>HC
                                                            </option>
                                                            <option value="5">Khác</option>
                                                        </select>
                                                        <input type="time"
                                                            class="form-control form-control-sm start-time-input mb-1"
                                                            value="{{ $group_code == '11' ? '06:00' : '07:15' }}"
                                                            {{ !$canEdit ? 'disabled' : '' }}>
                                                        <input type="time"
                                                            class="form-control form-control-sm end-time-input mb-1"
                                                            value="{{ $group_code == '11' ? '14:00' : '16:00' }}"
                                                            {{ !$canEdit ? 'disabled' : '' }}>
                                                        <div class="input-group input-group-sm">
                                                            <div class="input-group-prepend">
                                                                <span class="input-group-text"><i
...
<script>
    ...
    const personnelSkills = {
        @foreach ($personnelInfo as $pid => $s)
    ...
    $(document).ready(function() {
        ...
        $(document).on('change', '.shift-select', function() {
            ...
            const roomRow = $(this).closest('.room-row');
            let count;
            if (shift === '1') count = roomRow.data('n1');
            else if (shift === '2') count = roomRow.data('n2');
            else if (shift === '3') count = roomRow.data('n3');
            else if (shift === '6') count = roomRow.data('n4');
            else if (shift === '4') count = roomRow.data('nr');
                    ...
                    const res = await $.ajax({
                        ...
                        data: {
                            ...
                            stage_groups_code: $('select[name="group_code"]').val() || $row
                                .attr('data-group-code') || "{{ $group_code }}",
        ...
        async function autoAssign() {
            try {
                const result = await Swal.fire({
                    ...
                    text: 'Hệ thống sẽ dựa vào số lượng yêu cầu và xếp tự động theo cơ chế rải đều (round-robin) nhân sự vào các ca. Các thay đổi nhân sự chưa lưu trên màn hình sẽ bị ghi đè. Bạn có chắc chắn?',
                    icon: 'warning',
                    showCancelButton: true,
                    confirmButtonText: 'Đồng ý',
                    cancelButtonText: 'Hủy'
                ...
                $('.room-row').each(function() {
                    ...
                    let roomNameText = 'Công tác khác';
                    if ($roomRow.find('.room-name-cell b').length > 0) {
                        roomNameText = $roomRow.find('.room-name-cell b').text() + ' - ' +
        ...
        function renderSidebarData(data, currentDay, query = '') {
            ...
            $('.btn-toggle-has-assign').on('change', function(e) {
                ...
                $.ajax({
                    url: "{{ route('pages.assignment.maintenance.update_has_assignment') }}",
                    method: "POST",
                    data: {
                        _token: "{{ csrf_token() }}",
                        code: code,
