...
class StatusController extends Controller
{
        ...
        public function show()
        {
                ...
                $latestSub = DB::table('room_status as rs2')
                        ->join('room as r2', 'rs2.room_id', '=', 'r2.id')
                        ->where('rs2.is_daily_report', 0)
                        ->select('r2.production_group', DB::raw('MAX(rs2.id) as max_id'))
                        ->groupBy('r2.production_group');

                $lasestupdate = DB::table('room_status as rs')
                        ->joinSub($latestSub, 'ls', 'rs.id', '=', 'ls.max_id')
                ...
                $datas = DB::table('room')
                        ...
                        ->select(
                                ...
                                DB::raw("COALESCE(rs.start, '') as start_realtime"),
                                DB::raw("COALESCE(rs.end, '') as end_realtime"),
                        )
