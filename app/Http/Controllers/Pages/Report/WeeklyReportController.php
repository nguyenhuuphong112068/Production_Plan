...
class WeeklyReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $end_week = $start_week->copy()->endOfWeek(Carbon::SUNDAY);

        // ✅ Lấy off_days trước
        $offDays = DB::table('off_days')
            ->whereBetween('off_date', [
                $start_week->toDateString(),
                $end_week->toDateString()
            ])
            ->pluck('off_date')
            ->toArray();

        $totalWorkingDays = 0;
        $current = $start_week->copy();

        while ($current <= $end_week) {
            if (

                !in_array($current->toDateString(), $offDays)
            ) {
                $totalWorkingDays++;
            }
            $current->addDay();
        ...
        if (!$check) {
            $rooms = DB::table('room')
                ...
                ->select(
                    ...
                    DB::raw("3 as shift") // Mặc định shift = 3 khi khởi tạo
                )
                ->get();
